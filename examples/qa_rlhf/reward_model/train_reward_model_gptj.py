...
def create_comparison_dataset(path="CarperAI/openai_summarize_comparisons", split="train"):
    dataset = load_dataset(path, split=split) #Change this to csv
    pairs = []
    for sample in tqdm(dataset):
        ...
        if len(chosen_summary.split()) < 5 or len(rejected_summary.split()) < 5:
            continue
        pair["chosen"] = "Question: " + prompt + "\n" + "Answer: " + chosen_summary
        pair["rejected"] = "Question: " + prompt + "\n" + "Answer: "  + rejected_summary
        pairs.append(pair)
    return pairs

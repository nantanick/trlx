...
class AcceleratePPOTrainer(AccelerateRLTrainer):
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        while len(ppo_rl_elements) < num_rollouts:
            ...
            if self.config.model.model_arch_type == "seq2seq":
                start = 0
                attention_mask = sample_outputs[:, start:] != self.tokenizer.pad_token_id
            else:
            ...
            self.mean_kl = (kl * attention_mask[:, :-1].cpu()).sum(1).mean()
        ...
        if torch.distributed.is_initialized():
            self.mean_kl = torch.distributed.all_reduce(self.mean_kl)

        stats["kl"] = self.mean_kl
